...
The final and most important comparison between these two diffusions is the total heat. Here we can see that for the entire system there is a continious growth as time passes for the nonconserved case. Then, the conserved case oscillates around the zero mark. Since this graph is calculated as the difference between the initial heat and the current heat, there is almost no change since the value ranges from 
